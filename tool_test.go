...
func TestBuildDiscoveryResult(t *testing.T) {
	...
	result := buildDiscoveryResult(domain, subdomains)

	// Should have root domain + 3 subdomains = 4 domains
	assert.Len(t, result.Domains, 4)

	// Check root domain is present
	foundRoot := false
	...
	assert.True(t, foundRoot, "Root domain should be present")

	// Check all subdomains are present
	for _, subdomain := range subdomains {
		...
		assert.True(t, found, "Subdomain %s should be present", subdomain)
	}
}

func TestBuildDiscoveryResult_SkipDuplicateRoot(t *testing.T) {
	domain := "example.com"
	subdomains := []string{
		"example.com", // Duplicate root domain
		"api.example.com",
	}

	result := buildDiscoveryResult(domain, subdomains)

	// Should have root domain + 1 subdomain = 2 domains (not 3)
